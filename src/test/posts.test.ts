...
describe('Posts Controller Tests', () => {
  ...
  describe('Post Update', () => {
    test('Should update post as owner', async () => {
      ...
      const response = await request(app).put(`/posts/${testPostId}`).set('Authorization', `JWT ${accessToken}`).send(updateData);
    ...
    test('Should reject update without authentication', async () => {
      ...
      const response = await request(app).put(`/posts/${testPostId}`).send(updateData);
    ...
    test('Should reject update for non-existent post', async () => {
      ...
      const response = await request(app).put(`/posts/${fakeId}`).set('Authorization', `JWT ${accessToken}`).send({ name: 'Updated Name' });
    ...
    test('Should reject update by non-owner', async () => {
      ...
      const response = await request(app).put(`/posts/${testPostId}`).set('Authorization', `JWT ${anotherUserToken}`).send({ name: 'Unauthorized Update' });
  ...
  describe('Search and Advanced Queries', () => {
    ...
    test('Should validate date range during post operations', async () => {
      ...
      const updateResponse = await request(app).put(`/posts/${postId}`).set('Authorization', `JWT ${accessToken}`).send(invalidDatePost);
